...
void main() {
  ...
  group('AddTransactionDialog', () {
    testWidgets(
      'renders title, defaults to Expense, and loads expense categories sorted',
      (tester) async {
    ...
    testWidgets(
      'swicthing to Income reloads category list to income categories',
      (tester) async {
        await tester.pumpWidget(_wrap(db));
        await _openDialog(tester);
    ...
    testWidgets('date field opens the dashboard-styled calendar', (
      tester,
    ) async {
      await tester.pumpWidget(_wrap(db));
      await _openDialog(tester);
      ...
      expect(find.text('SELECT TRANSACTION DATE'), findsOneWidget);
    ...
    testWidgets(
      'saving a valid expense persists it, assigns the category, closes, and calls onAdded',
      (tester) async {
    ...
    testWidgets(
      'saving a valid income transaction stores TransactionType.income',
      (tester) async {
        await tester.pumpWidget(_wrap(db));
        await _openDialog(tester);
        await tester.tap(find.text('Income'));
    ...
    testWidgets(
      'amount field rejects more than 2 decimal places via input formatter',
      (tester) async {
        await tester.pumpWidget(_wrap(db));
        await _openDialog(tester);
        final amountField = find.byType(TextFormField).at(1);
        ...
        expect(
          ...
          anyOf(equals('100.99'), isNot(equals('100.999'))),
        );
      },
    );
  });
