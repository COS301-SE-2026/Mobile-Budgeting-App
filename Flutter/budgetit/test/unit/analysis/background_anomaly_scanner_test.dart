...
@GenerateMocks(
  [AppDatabase, TransactionDao, CategoryDao],
) //ignore errors once built the mocks will be generated and tests willl run TRUST
...
void main() {
  ...
  setUp(() {
    ...
    when(
      mockTxDao.getTransactionsByDateRange(any, any),
    ).thenAnswer((_) async => []);
  ...
  group('BackgroundAnomalyScanner', () {
    ...
    test(
      'scan with no history reutrns empty anomalies and null predicition',
      () async {
        await scanner.scan();
        expect(scanner.anomalies, isEmpty);
        expect(scanner.prediction, isNull);
    ...
    test('isStale returns true when maxAge is exceeded', () async {
      ...
      expect(scanner.isStale(maxAge: const Duration(milliseconds: 1)), isTrue);
    });
    ...
    test('anomalies list is unmodifiable', () async {
      ...
      expect(
        () => (scanner.anomalies as dynamic).clear(),
