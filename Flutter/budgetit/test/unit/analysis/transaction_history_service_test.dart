...
void main() {
  ...
  group('TransactionHistoryService', () {
    ...
    test('getMonthlyHistory months are ordered oldest to newest', () async {
      when(
        mockTxDao.getTransactionsByDateRange(any, any),
      ).thenAnswer((_) async => []);
    ...
    test(
      'getNonEmptyMonthlyHistory filters out months with no transactions',
      () async {
        when(
          mockTxDao.getTransactionsByDateRange(any, any),
        ).thenAnswer((_) async => []);
    ...
    test('summary has zero totals when no transactions exist', () async {
      when(
        mockTxDao.getTransactionsByDateRange(any, any),
      ).thenAnswer((_) async => []);
