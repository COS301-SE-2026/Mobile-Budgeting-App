...
void main() {
  ...
  group('_parseDate', () {
    ...
    test('throws FormatException for unrecognized format', () {
      expect(
        () => parser.testParseDate('not-a-date'),
  ...
  group('_parseAmount', () {
    ...
    test('parse negative amount', () {
      expect(
        parser.testParseAmount('-450.00'),
        equals(Decimal.parse('-450.00')),
      );
    });

    test('parses amount with commas', () {
      expect(
        parser.testParseAmount('1,234.00'),
        equals(Decimal.parse('1234.00')),
      );
    });

    test('parse amount in brackets as negative', () {
      expect(
        parser.testParseAmount('(200.00)'),
        equals(Decimal.parse('-200.00')),
      );
    });

    test('parse dollar amount/no prefix amounts ', () {
      expect(
        parser.testParseAmount('1000.00'),
        equals(Decimal.parse('1000.00')),
      ); // parsePDFLines removes $ so eh, test for no prefix
    });
    ...
    test('parse larger amount with comma', () {
      //because what if yk
      expect(
        parser.testParseAmount('10,000.00'),
        equals(Decimal.parse('10000.00')),
      );
      expect(
        parser.testParseAmount('100,000.00'),
        equals(Decimal.parse('100000.00')),
      );
    });
  });
  ...
  group('CSV parsing', () {
    ...
    test('different transaction produce different hashes', () async {
      ...
      final results = await parser.parse(file.path);
      expect(
        results[0].deduplicationHash,
        isNot(equals(results[1].deduplicationHash)),
      );
    });
  ...
  group('parse() routing', () {
    test('throws UnsupportedError for unsupported files', () async {
      expect(
        () async => await parser.parse('/path/to/file.xlsx'),
    ...
    test('throws unsupportedError for no file extension', () async {
      expect(
        () async => await parser.parse('/path/to/file'),
        throwsA(isA<UnsupportedError>()),
      );
    });
