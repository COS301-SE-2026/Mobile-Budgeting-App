...
import 'services/ai/transaction_classifier/fake_text_embedder.dart';
...
class _AiClassifierDemoScreenState extends State<AiClassifierDemoScreen> {
  ...
  late final FakeTextEmbedder _embedder;
  ...
  void initState() {
    ...
    _embedder = FakeTextEmbedder(modelVersion: 'controlled-demo-embedder-v1');
  ...
  Future<void> _initialize() async {
    try {
      ...
      debugPrint('Demo embedder initialization failed: $error');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: ListView(
        ...
        children: [
          Text(
            'Demo embedder: ${_embedder.modelVersion}',
          ...
          Text(
            _initializing
                ...
                : 'Classifier ready — controlled test embedder',
          ),

          const SizedBox(height: 8),

          const Text(
            'Workflow demonstration only. Category accuracy will be provided '
            'by the BGE ONNX model.',
